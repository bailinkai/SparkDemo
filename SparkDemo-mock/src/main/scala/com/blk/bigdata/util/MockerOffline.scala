...
import com.blk.bigdata.model.{CityInfo, ProductInfo, UserInfo, UserVisitAction}
...
object MockerOffline {
  ...
  val searchKeywordsOptions = RandomOptions(RanOpt("手机", 30), RanOpt("笔记本", 70), RanOpt("内存", 70), RanOpt("i7", 70), RanOpt("苹果", 70), RanOpt("王者荣耀", 70))
  ...
  def insertHive(sparkSession: SparkSession, tableName: String, dataFrame: DataFrame): Unit = {
    sparkSession.sql("use " + ConfigurationUtil.readFile("hive.database"))
  ...
  def mockUserAction() = {
    ...
    val endDate = new SimpleDateFormat("yyyy-MM-dd").parse("2018-12-30")
